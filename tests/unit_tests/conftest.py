...
def test_friendly_whitelist(request: FixtureRequest):
    ...
    with patch("daq_config_server._core.whitelist.requests.get") as mock_request:
        ...
        if request.node.get_closest_marker("use_threading"):  # type: ignore
            with patch(
                "daq_config_server._core.whitelist.WHITELIST_REFRESH_RATE_S", new=0
            ):
                yield
                get_whitelist().stop()
        else:
            with (
                patch("daq_config_server._core.whitelist.Thread"),
                patch("daq_config_server._core.whitelist.WhitelistFetcher.stop"),
            ):
                yield
