...
class TemperatureControllersConfig(ConfigModel):
    ...
    def from_xpdf_parameters(cls, contents: str) -> Self:
        ...
        devices = config_dict["configuration"]["devices"]
        cobra = TemperatureControllerParams.model_validate(devices["cobra"])
        blower = TemperatureControllerParams.model_validate(devices["blower"])
        cryostream = TemperatureControllerParams.model_validate(devices["cryostream"])
